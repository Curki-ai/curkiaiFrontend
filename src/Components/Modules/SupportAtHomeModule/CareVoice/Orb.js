 ...
 * GLSL shader: seven morphing ovals + noise rings in polar coords, sampled from
 * a Perlin texture, mapped through a 4-colour ramp (black -> color1 -> color2 ->
 * white). Reacts to mic level via inputVolumeRef / outputVolumeRef.
 *
 * Curki uses the brand-purple ramp by default; pass `colors` to override.
 */

export function Orb({
    colors = ["#6c4cdc", "#c9b6fb"], // [darker, lighter] — Curki brand purple
    colorsRef,
    resizeDebounce = 100,
    seed = 1234,
    agentState = null,
    ...
    return (
        <div className={className ?? "cv-orb-fill"}>
            <Canvas
...
// Map grayscale value to a 6-color ramp for a richer, multi-hue orb
vec3 colorRamp6(float t, vec3 c0, vec3 c1, vec3 c2, vec3 c3, vec3 c4, vec3 c5) {
...
void main() {
    ...
    // On-brand premium ramp: deep indigo -> purple -> blue -> lavender -> warm
    // highlight -> pale lavender. No pure black (muddy) and no pure white (blobs).
    // Purple stays dominant; blue (#328DEE) + a soft amber are subtle accents.
    vec3 c0 = vec3(0.141, 0.090, 0.325);  // deep indigo (#241753)
    vec3 c1 = uColor1;                    // brand purple (#6c4cdc)
    vec3 c2 = vec3(0.196, 0.553, 0.933);  // blue accent (#328DEE)
    vec3 c3 = uColor2;                    // lavender (#c9b6fb)
    vec3 c4 = vec3(0.953, 0.682, 0.467);  // soft warm amber highlight (#f3ae77)
    vec3 c5 = vec3(0.937, 0.918, 1.000);  // pale lavender, NOT white (#efeaff)

    // Convert grayscale to the ramp, with a gamma bias toward the purple (lower)
    // range so the orb reads purple-dominant and bright/white areas stay small.
    float luminance = mix(color.r, 1.0 - color.r, uInverted);
    luminance = pow(clamp(luminance, 0.0, 1.0), 1.35);
    color.rgb = colorRamp6(luminance, c0, c1, c2, c3, c4, c5); // Apply the multi-hue ramp
